...
function setup(): Promise<Env> {
    ...
    return agile_keychain_crypto
        .generateMasterKey(VAULT_PASS, VAULT_PASS_ITERATIONS)
        .then(keyList => {
            let keys = agile_keychain.convertKeys(keyList.list);
            return asyncutil.all2([
                localStore.saveKeys(keys, VAULT_PASS_HINT),
                cloudStore.saveKeys(keys, VAULT_PASS_HINT),
            ]);
        })
        .then(() => {
            return asyncutil.all2([
                localStore.unlock(VAULT_PASS),
...
testLib.addTest('sync keys and password hint from cloud to local', assert => {
    ...
    return setup()
        ...
        .then(() => {
            return asyncutil.all2([
                env.localStore.listKeys(),
                env.localStore.passwordHint(),
            ]);
        })
        .then(keyAndHint => {
...
testLib.addTest('sync items from cloud to local', assert => {
    ...
    return setup()
        ...
        .then(storeItems => {
            ...
            assert.ok(
                sync.itemUpdateTimesEqual(
                    storeItems[0].createdAt,
                    item.createdAt
                )
            );
...
testLib.addTest('merge local and cloud item updates', assert => {
    ...
    return setup()
        ...
        .then(() => {
            return asyncutil.all2([
                env.localStore.loadItem(item.uuid),
                env.cloudStore.loadItem(item.uuid),
            ]);
        })
        .then(items => {
...
testLib.addTest('sync item deletion from cloud to local', assert => {
    ...
    return setupWithItem()
        ...
        .then(items => {
            ...
            return env.localStore.getLastSyncedRevision(
                item.uuid,
                CLOUD_STORE_ID
            );
        })
...
testLib.addTest('item deleted in cloud and locally', assert => {
    ...
    return setupWithItem()
        ...
        .then(() => {
            let deletedLocally = env.localStore
                .loadItem(uuid)
                .then(item => item.item.remove());
            let deletedInCloud = env.cloudStore
                .loadItem(uuid)
                .then(item => item.item.remove());
    ...
    return setup()
        ...
        .then(items => {
            assert.equal(
                items.length,
                ITEM_COUNT,
                'synced expected number of items'
            );
        });
});
    ...
    return setupWithItem()
        ...
        .then(() => {
            ...
            env.cloudStore.loadItem = uuid => {
                if (uuid === item.uuid) {
                    return Promise.reject<item_store.ItemAndContent>(
                        'Could not load item'
                    );
                } else {
