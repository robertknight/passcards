...
testLib.addTest('saves and syncs items', assert => {
	...
	return vfs_util.mktemp(fs, '/', 'sync-integration-test-XXX.agilekeychain').then(vaultPath => {
		return agile_keychain.Vault.createVault(fs, vaultPath, TEST_PASS, '', 100, keyAgent);
	}).then(vault => {
