    ...
    testLib.addTest(fsName + ': write conflict detection', assert => {
        ...
        return createFs()
            ...
            .then(stat => {
                ...
                var attemptA = fs
                    .write('test-file-conflict', 'content-v2-a', writeOpts)
                    .then(() => true)
                    .catch(() => false);
                var attemptB = fs
                    .write('test-file-conflict', 'content-v2-b-b', writeOpts)
                    .then(() => true)
                    .catch(() => false);
                return Promise.all([attemptA, attemptB]);
            })
            .then(ok => {
                var successCount = ok.reduce(
                    (total, ok) => (ok ? total + 1 : total),
                    0
                );
            ...
            .then(content => {
                assert.ok(
                    content == 'content-v2-a' || content == 'content-v2-b-b'
                );
            });
    });
