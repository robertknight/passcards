    ...
    write(
        ...
        fs.writeFile(tempPath, content, error => {
            ...
            try {
                ...
                if (
                    FileVFS.statToRevision(fileStat) !== options.parentRevision
                ) {
                    result.reject(
                        new vfs.VfsError(vfs.ErrorType.Conflict, path)
                    );
                }
        ...
        fs.mkdir(this.absPath(path), 511 /* 0777 */, err => {
            if (err) {
                if (err.code === 'ENOENT') {
                    ...
                    this.mkpathInternal(Path.dirname(path), true)
                        .then(() => {
