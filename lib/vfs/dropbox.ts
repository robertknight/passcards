...
export class DropboxVFS implements vfs.VFS {
	...
	search(namePattern: string, cb: (err: Error, files: vfs.FileInfo[]) => any): void {
		this.client.filesSearch({
			path: '',
			query: namePattern,
			start: 0,
			max_results: 100,
	...
	write(path: string, contents: string, options?: vfs.WriteOptions): Promise<vfs.FileInfo> {
