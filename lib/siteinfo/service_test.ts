...
var urlFetcher: site_info_service.UrlFetcher = {
    fetch(url: string): Promise<site_info_service.UrlResponse> {
        ...
        return http_client
            .request('GET', urlLib.format(parsedUrl))
            .then(reply => {
                return {
                    status: reply.status,
                    body: reply.body,
...
var TEST_SITE_PATH = path.join(
    stringutil.replaceLast(path.dirname(module.filename), 'build/', ''),
...
function extractIconLinks(
    contentFilePath: string
): site_info_service.PageLink[] {
    var extractor = new site_info_service.PageLinkFetcher(urlFetcher);
    var content = fs
        .readFileSync(path.join(TEST_SITE_PATH, contentFilePath))
        .toString();
...
testLib.addTest('read PNG icon', assert => {
    var iconPath = path.join(
        TEST_SITE_PATH,
        'wikipedia/standard-icons/apple-touch-icon.png'
    );
...
testLib.addTest('read BMP icon', assert => {
    var iconPath = path.join(
        TEST_SITE_PATH,
        'wikipedia/linked-icons/favicon-16x16.bmp'
    );
    var data = new Uint8Array(<any>fs.readFileSync(iconPath));
...
testLib.addTest('read ICO icon', assert => {
    var iconPath = path.join(
        TEST_SITE_PATH,
        'wikipedia/standard-icons/favicon.ico'
    );
    var data = new Uint8Array(<any>fs.readFileSync(iconPath));
    ...
    var expectedIcons = [
        {
            width: 16,
            height: 16,
            data: 'wikipedia/linked-icons/favicon-16x16.bmp',
        },
        {
            width: 32,
            height: 32,
            data: 'wikipedia/linked-icons/favicon-32x32.bmp',
        },
        {
            width: 48,
            height: 48,
            data: 'wikipedia/linked-icons/favicon-48x48.bmp',
        },
    ];

    icons.forEach((icon, index) => {
        var expectedIcon = expectedIcons[index];
        var expectedData = fs.readFileSync(
            path.join(TEST_SITE_PATH, expectedIcon.data)
        );
        assert.equal(icon.width, expectedIcon.width);
        assert.equal(icon.height, expectedIcon.height);
        assert.deepEqual(
            collection_util.bufferToArray(icon.data),
...
testLib.addTest('image decode error', assert => {
    ...
    try {
        site_info_service.iconFromData(
            'foo.png',
            new Uint8Array(invalidIconData)
        );
        assert.ok(false);
    } catch (ex) {
...
testLib.addTest('extract largest ICO icon', assert => {
    ...
    var expectedData = fs.readFileSync(
        path.join(TEST_SITE_PATH, 'wikipedia/linked-icons/favicon-48x48.bmp')
    );
    assert.deepEqual(
        collection_util.bufferToArray(icon.data),
        collection_util.bufferToArray(expectedData)
...
function serveAndFetchIcons(
    port: number,
    siteRoot: string,
    queryPath: string
): Promise<ServeFetchResult> {
