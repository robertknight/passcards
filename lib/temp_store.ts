...
export class Store implements item_store.SyncableStore {
    ...
    private lastSyncedRevisions: Map<
        string,
        Map<string, item_store.RevisionPair>
    >;
    ...
    loadItem(uuid: string, revision?: string) {
        return this.checkUnlocked().then(() => {
            ...
            if (items[0].isTombstone()) {
                return Promise.reject<item_store.ItemAndContent>(
                    new Error('Item has been deleted')
                );
            }
            if (!revision) {
    ...
    getContent(item: item_store.Item) {
        return this.checkUnlocked().then(() => {
            if (this.content.has(item.revision)) {
                ...
                return Promise.reject<item_store.ItemContent>(
                    new Error('No such item')
                );
            }
        });
    ...
    getRawDecryptedData(item: item_store.Item) {
        return Promise.reject<string>(
            new Error('Not implemented in TempStore')
        );
    }
    ...
    clear() {
        ...
        this.lastSyncedRevisions = new Map<
            string,
            Map<string, item_store.RevisionPair>
        >();
        return Promise.resolve<void>(null);
    ...
    setLastSyncedRevision(
        ...
        if (!this.lastSyncedRevisions.has(storeID)) {
            this.lastSyncedRevisions.set(
                storeID,
                new Map<string, item_store.RevisionPair>()
            );
        }
        if (revision) {
