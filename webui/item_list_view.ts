...
export class Item extends react.Component<ItemProps, {}> {
    ...
    shouldComponentUpdate(nextProps: ItemProps, nextState: {}) {
        ...
        // (see createListItem())
        return (
            reactutil.objectChanged(this.props, nextProps, 'onSelected') ||
...
class ItemList extends react.Component<ItemListProps, ItemListState> {
    ...
    private ensureItemVisible(index: number) {
        ...
        if (this.state.focusedIndex <= this.state.visibleIndexes.first) {
            scrollDelta =
                this.state.focusedIndex - this.state.visibleIndexes.first - 1;
        } else if (this.state.focusedIndex >= this.state.visibleIndexes.last) {
            scrollDelta =
                this.state.visibleIndexes.last - this.state.focusedIndex + 1;
        }
    ...
    componentWillReceiveProps(nextProps: ItemListProps) {
        if (
            !shallow_equals(this.props.items, nextProps.items) ||
    ...
    render() {
        ...
        var listItems = this.state.matchingItems
            .map((item, index) => {
                var isVisible =
                    index >= renderedIndexes.first &&
    ...
    private renderedIndexes() {
        ...
        return {
            ...
            last: Math.min(
                this.state.matchingItems.length - 1,
                this.state.visibleIndexes.last + runway
            ),
        };
    }
    ...
    private updateMatchingItems(props: ItemListProps) {
        ...
        if (props.filter) {
            ...
            matchingItems = item_search.filterItemsByUrl(
                props.items,
                props.filterUrl
            );
            if (matchingItems.length > 0) {
...
class ItemListToolbar extends react.Component<ItemListToolbarProps, {}> {
    ...
    private fieldInput() {
        return <HTMLInputElement>react_dom.findDOMNode(
        ...
        return div(
            ...
            div(
                ...
                toolbar.createButton({
                    ...
                    onClick: () => {
                        var event = {
                            itemRect: (<HTMLElement>react_dom.findDOMNode(
