...
class AppView extends react.Component<AppViewProps, AppViewState> {
    ...
    private showError(error: Error, context?: string) {
        ...
        var status = new status_message.Status(
            status_message.StatusType.Error,
            message
        );
    ...
    private autofill(item: item_store.Item) {
        this.props.services.autofiller
            .autofill(item)
            .then(result => {
                if (result.count > 0) {
                ...
                onUnlock: () => {
                    ...
                    itemStoreState.syncer
                        ...
                        .catch(err => {
                            this.showError(
                                new Error(
                                    `Unable to sync items: ${err.toString()}`
                                )
                            );
                        });
                },
        ...
        if (this.props.appState.state.isSigningIn) {
            activeDialog = auth_dialog.AuthDialogF({
                ...
                onComplete: credentials => {
                    if (credentials) {
                        this.props.appState.state.onReceiveCredentials(
                            credentials
                        );
                    }
    ...
    private renderToasters() {
        ...
        if (syncState && syncState.state !== sync.SyncState.Idle) {
            toasters.push(
                toaster.ToasterF({
                    key: 'sync-toaster',
                    message: 'Syncing...',
        ...
        return reactutil.TransitionGroupF(
            <any>{ key: 'toasterList' },
            toasters
        );
    }
        ...
        if (this.itemStoreState().selectedItem) {
            ...
            detailsView = details_view.DetailsViewF({
                ...
                onSave: updatedItem => {
                    ...
                    // transitioned out
                    var SAVE_DELAY = 1000;
                    delay(null, SAVE_DELAY)
                        .then(() => {
                            return updatedItem.item.saveTo(
                                this.itemStoreState().store
                            );
                        })
                        .then(() => {
                        ...
                        .then(() => {
                            this.showStatus(
                                new status_message.Status(
                                    status_message.StatusType.Success,
    ...
    private createNewItemTemplate() {
        ...
        Object.keys(accountFreq).forEach(account => {
            if (
                !defaultAccount ||
                accountFreq[account] > accountFreq[defaultAccount]
            ) {
        ...
        var currentUrlProtocol = url.parse(this.itemStoreState().currentUrl)
            .protocol;

        if (AUTOFILL_URL_SCHEMES.indexOf(currentUrlProtocol) !== -1) {
            builder.setTitle(
                url_util.topLevelDomain(this.itemStoreState().currentUrl)
            );
        ...
        menuItems = menuItems.concat([
            {
                ...
                onClick: () => {
                    return this.props.services.keyAgent
                        .forgetKeys()
                        .then(() => {
            ...
            },
            {
                label: 'Switch Store',
                onClick: () => {
                    this.props.services.settings.clear(
            ...
            },
            {
                ...
                onClick: () => {
                    var win = window.open(
                        'https://robertknight.github.io/passcards',
                        '_blank'
                    );
                    win.focus();
                },
