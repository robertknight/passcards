...
testLib.addTest('should display field actions when focused', assert => {
    ui_test_utils.runReactTest(element => {
        ...
        let usernameField = field.refs['usernameField'] as react.Component<
            any,
            any
        >;
        let usernameFieldInput = <HTMLElement>react_dom.findDOMNode(
            usernameField.refs['textField']
        );
        let passwordField = field.refs['passwordField'] as react.Component<
            any,
            any
        >;
        let passwordFieldInput = <HTMLElement>react_dom.findDOMNode(
            passwordField.refs['textField']
        );
        ...
        let actionButtons = (field: react.Component<{}, {}>) =>
            scryRenderedComponentsWithType(field, button.Button);
